...
CREATE TABLE Products(ProductID INT PRIMARY KEY auto_increment,ProductName VARCHAR(50),Description TEXT,Price DECIMAL(10,2));
...
CREATE TABLE OrderDetails(OrderDetailID INT PRIMARY KEY,OrderID INT,FOREIGN KEY(OrderID) REFERENCES Orders(OrderID) ON DELETE CASCADE,ProductID INT,
FOREIGN KEY(ProductID) REFERENCES Products(ProductID) ON DELETE CASCADE,Quantity INT);
CREATE TABLE Inventory(InventoryID INT PRIMARY KEY,ProductID INT,FOREIGN KEY(ProductID) REFERENCES Products(ProductID) ON DELETE CASCADE,
...
(113,'Aara','sree','aarasree14@gmail.com','9133456780','1/2 LGO Colony,Madurai,636122');
